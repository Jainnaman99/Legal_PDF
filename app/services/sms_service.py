...
class SmsService:
    ...
    def send_otp(self, to_number: str, otp: str) -> None:
        if not settings.TWILIO_ACCOUNT_SID or not settings.TWILIO_AUTH_TOKEN:
            logger.warning("[SmsService] Twilio not configured — OTP for %s: %s", to_number, otp)
            return
        ...
        try:
            ...
            client.messages.create(
                body=f"Your Haryana Legal Knowledge System password reset OTP is: {otp}. Valid for 10 minutes.",
                from_=settings.TWILIO_PHONE_NUMBER,
                to=to_number,
            )
            logger.info("[SmsService] OTP sent to %s", to_number)
        except ImportError:
            ...
            logger.error("[SmsService] Failed to send OTP to %s: %s", to_number, exc)
            raise
