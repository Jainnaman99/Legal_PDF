-- Allows file replacement (and resubmit) on documents that have been sent
-- back to the uploader via the unlock-request workflow (status = 'returned'),
-- in addition to the existing pending/rejected cases. Otherwise identical to
-- patch_sp_replace_pdf_clear_annotations.sql.
--
-- Run via mysql client:
--   mysql -u <user> -p legal_pdf < scripts/patch_sp_replace_pdf_file_allow_returned.sql
...
SELECT 'sp_replace_pdf_file now allows returned documents.' AS status;
