-- ============================================================
-- Legal PDF — MySQL 8.0 Schema
-- Run via: mysql -u root -p legal_pdf < scripts/mysql_schema.sql
-- ============================================================

CREATE DATABASE IF NOT EXISTS legal_pdf CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE legal_pdf;
...
DROP TABLE IF EXISTS pdf_document_department_links;
DROP TABLE IF EXISTS pdf_document_relationships;
DROP TABLE IF EXISTS pdf_document_approvals;
DROP TABLE IF EXISTS pdf_pages;
DROP TABLE IF EXISTS pdf_document_tags;
DROP TABLE IF EXISTS pdf_documents;
DROP TABLE IF EXISTS nodal_officer_departments;
...
CREATE TABLE users (
    ...
    role_id              INT,
    department_id        VARCHAR(500),   -- CSV e.g. "1,2" for nodal officers
    created_at           DATETIME(6)  NOT NULL DEFAULT UTC_TIMESTAMP(6),
    updated_at           DATETIME(6)  NOT NULL DEFAULT UTC_TIMESTAMP(6),
    CONSTRAINT fk_users_role FOREIGN KEY (role_id) REFERENCES roles(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE nodal_officer_departments (
    id            INT AUTO_INCREMENT PRIMARY KEY,
    user_id       INT NOT NULL,
    department_id INT NOT NULL,
    created_at    DATETIME(6) NOT NULL DEFAULT UTC_TIMESTAMP(6),
    UNIQUE KEY uq_nod_user_dept (user_id, department_id),
    CONSTRAINT fk_nod_user FOREIGN KEY (user_id)       REFERENCES users(id),
    CONSTRAINT fk_nod_dept FOREIGN KEY (department_id) REFERENCES departments(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE pdf_documents (
    ...
    no_of_orders         INT,
    keywords             TEXT,
    is_repealed          TINYINT(1)   NOT NULL DEFAULT 0,
    uploaded_by          INT          NOT NULL,
    created_at           DATETIME(6)  NOT NULL DEFAULT UTC_TIMESTAMP(6),
    CONSTRAINT fk_pdf_uploader FOREIGN KEY (uploaded_by)      REFERENCES users(id),
    CONSTRAINT fk_pdf_dept     FOREIGN KEY (department_id)    REFERENCES departments(id),
    CONSTRAINT fk_pdf_type     FOREIGN KEY (document_type_id) REFERENCES document_types(id),
    CONSTRAINT chk_pdf_status  CHECK (status IN ('pending','approved','rejected'))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE pdf_document_tags (
    pdf_id INT NOT NULL,
    tag_id INT NOT NULL,
    PRIMARY KEY (pdf_id, tag_id),
    CONSTRAINT fk_pdt_pdf FOREIGN KEY (pdf_id) REFERENCES pdf_documents(id) ON DELETE CASCADE,
    CONSTRAINT fk_pdt_tag FOREIGN KEY (tag_id) REFERENCES tags(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE pdf_document_approvals (
    id               INT AUTO_INCREMENT PRIMARY KEY,
    pdf_id           INT          NOT NULL,
    ...
    acted_at         DATETIME(6)  NOT NULL DEFAULT UTC_TIMESTAMP(6),
    CONSTRAINT fk_pda_pdf      FOREIGN KEY (pdf_id)      REFERENCES pdf_documents(id),
    CONSTRAINT fk_pda_approver FOREIGN KEY (approver_id) REFERENCES users(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE pdf_document_relationships (
    id                INT AUTO_INCREMENT PRIMARY KEY,
    source_pdf_id     INT         NOT NULL,
    target_pdf_id     INT         NOT NULL,
    relationship_type VARCHAR(50) NOT NULL DEFAULT 'related',
    created_at        DATETIME(6) NOT NULL DEFAULT UTC_TIMESTAMP(6),
    CONSTRAINT fk_pdr_source FOREIGN KEY (source_pdf_id) REFERENCES pdf_documents(id),
    CONSTRAINT fk_pdr_target FOREIGN KEY (target_pdf_id) REFERENCES pdf_documents(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE pdf_document_department_links (
    id              INT AUTO_INCREMENT PRIMARY KEY,
    pdf_id          INT         NOT NULL,
    department_id   INT         NOT NULL,
    ...
    annotations_json LONGTEXT,
    created_at      DATETIME(6) NOT NULL DEFAULT UTC_TIMESTAMP(6),
    UNIQUE KEY uq_pdf_dept_link (pdf_id, department_id),
    CONSTRAINT fk_pddl_pdf    FOREIGN KEY (pdf_id)        REFERENCES pdf_documents(id),
    CONSTRAINT fk_pddl_dept   FOREIGN KEY (department_id) REFERENCES departments(id),
    CONSTRAINT fk_pddl_linker FOREIGN KEY (linked_by)     REFERENCES users(id),
    CONSTRAINT fk_pddl_rev    FOREIGN KEY (reviewed_by)   REFERENCES users(id),
...
CREATE TABLE pdf_pages (
    ...
    created_at       DATETIME(6)  NOT NULL DEFAULT UTC_TIMESTAMP(6),
    CONSTRAINT fk_pp_pdf FOREIGN KEY (pdf_document_id) REFERENCES pdf_documents(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE user_login_logs (
    ...
    ip_address VARCHAR(45),
    created_at DATETIME(6)  NOT NULL DEFAULT UTC_TIMESTAMP(6),
    CONSTRAINT fk_ull_user FOREIGN KEY (user_id) REFERENCES users(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE admin_login_otps (
    ...
    is_used    TINYINT(1)   NOT NULL DEFAULT 0,
    created_at DATETIME(6)  NOT NULL DEFAULT UTC_TIMESTAMP(6),
    CONSTRAINT fk_alo_user FOREIGN KEY (user_id) REFERENCES users(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE INDEX ix_pdf_docs_status      ON pdf_documents(status);
CREATE INDEX ix_pdf_docs_uploaded_by ON pdf_documents(uploaded_by);
CREATE FULLTEXT INDEX ix_pdf_pages_fts ON pdf_pages(page_text);
...
-- ── Stored Procedures ─────────────────────────────────────────
-- Helper macro: dept_name_subquery(alias)
-- Returns pipe-separated department names from a CSV department_id column.
...
INSERT INTO roles (name, description) VALUES
    ...
    ('nodal Officer', 'Nodal officer managing departments');

INSERT INTO document_types (name, description, is_active) VALUES
    ('Act',               'Legislative Acts',                     1),
    ('Amendment',         'Amendments to existing acts',          1),
    ('Notification',      'Government notifications',             1),
    ('Circular',          'Official circulars',                   1),
    ('Policy',            'Government policies',                  1),
    ('Rules & Regulations','Rules and Regulations',              1),
    ('Order/Gazette',     'Orders and Gazette notifications',     1);
