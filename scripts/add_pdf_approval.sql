-- ============================================================
-- Adds approval workflow to pdf_documents:
--   - status column (pending | approved | rejected)
--   - pdf_document_approvals table (audit trail)
--   - sp_review_pdf_document   (approve / reject action)
--   - sp_get_pending_pdfs      (approver queue)
--   - Updates all PDF-returning SPs to include status +
--     latest_approval in the result set.
--
-- Run via sqlcmd:
--   sqlcmd -S 10.0.160.80 -U sa -P sa@123 -i scripts\add_pdf_approval.sql
-- ============================================================
...
-- ─────────────────────────────────────────────
-- 1. Add status column to pdf_documents
-- ─────────────────────────────────────────────

IF NOT EXISTS (
    SELECT 1 FROM sys.columns
    WHERE object_id = OBJECT_ID('dbo.pdf_documents') AND name = 'status'
)
    ALTER TABLE dbo.pdf_documents
        ADD status NVARCHAR(20) NOT NULL
            CONSTRAINT DF_pdf_status    DEFAULT 'pending',
            CONSTRAINT CK_pdf_status    CHECK (status IN ('pending', 'approved', 'rejected'));
GO
...
IF OBJECT_ID('dbo.pdf_document_approvals', 'U') IS NULL
CREATE TABLE dbo.pdf_document_approvals
(
    ...
    acted_at     DATETIME2     NOT NULL CONSTRAINT DF_approval_acted_at DEFAULT GETDATE(),
    CONSTRAINT PK_pdf_approvals  PRIMARY KEY (id),
    CONSTRAINT CK_approval_action CHECK (action IN ('approved', 'rejected')),
    CONSTRAINT FK_approval_pdf   FOREIGN KEY (pdf_id)
...
-- ─────────────────────────────────────────────
-- 4. sp_get_pending_pdfs  (approver queue)
-- ─────────────────────────────────────────────
...
-- ─────────────────────────────────────────────
-- 5. Update existing PDF SPs to include status
--    and latest_approval
-- ─────────────────────────────────────────────
...
BEGIN
    ...
    INSERT INTO dbo.pdf_documents (
        ...
        version_no, department_id, document_type_id, description, status
    ) VALUES (
        @filename, @original_filename, @file_path, @file_size, @uploaded_by,
        @document_name, @reference_number, @issue_date, @effective_from,
        @gazette_reference, @legal_authority, @short_title, @valid_until,
        @sector_domain, @implementing_agency, @next_review_date, @rule_making_authority,
        @version_no, @department_id, @document_type_id, @description, 'pending'
    );
...
CREATE OR ALTER PROCEDURE dbo.sp_list_all_pdfs
    @skip   INT = 0,
    @limit  INT = 100,
    @status NVARCHAR(20) = NULL    -- optional filter: pending | approved | rejected
AS
BEGIN
    SET NOCOUNT ON;
    SELECT
